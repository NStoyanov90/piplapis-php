...
abstract class Field
{
    ...
    private function internal_reducecb_buildattrslist($res, $x)
    {
        if (is_array($x) && count($x) > 0)
        {
            ...
            if (isset($x[$keys[0]]))
            {
                ...
                {
                    $val = APIUtil::piplapi_datetime_to_str($val);
                }
    ...
    public static function from_array($clsname, $d)
    {
        ...
        {
            if (APIUtil::piplapi_string_startswith($key, '@'))
            {
                $key = substr($key, 1);
            }
            ...
            if ($key == 'last_seen')
            {
                $val = APIUtil::piplapi_str_to_datetime($val);
            }
            ...
            if ($key == 'date_range')
            {
                // DateRangeField has its own from_array implementation
                $val = DateRangeField::from_array($val);
            }
    ...
    public function to_array()
    {
        ...
        {
            $d['@valid_since'] = APIUtil::piplapi_datetime_to_str($this->valid_since);
        }
        if (!empty($this->last_seen))
        {
            $d['@last_seen'] = APIUtil::piplapi_datetime_to_str($this->last_seen);
        }
