...
class DateRangeField
{
    ...
    public function __toString()
    {
        ...
        if($this->start && $this->end) {
            return sprintf('%s - %s', APIUtil::piplapi_date_to_str($this->start),
                APIUtil::piplapi_date_to_str($this->end));
        } elseif($this->start) {
            return APIUtil::piplapi_date_to_str($this->start);
        }
        return APIUtil::piplapi_date_to_str($this->end);
    }
    ...
    public static function from_years_range($start_year, $end_year)
    {
        ...
        return new DateRangeField($newstart, $newend);
    }
    ...
    public static function from_array($d)
    {
        ...
        if($newstart) {
            $newstart = APIUtil::piplapi_str_to_date($newstart);
        }
        if($newend){
            $newend = APIUtil::piplapi_str_to_date($newend);
        }
        return new DateRangeField($newstart, $newend);
    }
    ...
    public function to_array()
    {
        ...
        $d = array();
        if($this->start) {
            $d['start'] = APIUtil::piplapi_date_to_str($this->start);
        }
        if($this->end){
            $d['end'] = APIUtil::piplapi_date_to_str($this->end);
        }
        return $d;
    }
}
