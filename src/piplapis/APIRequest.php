...
class APIRequest
{
    ...
    // $request = new PiplApi_SearchAPIRequest(array('person' => new Person(array('fields' => $fields))));
    // $response = $request->send();
    //
    ...
    static function get_default_configuration()
    {
        if (!isset(self::$default_configuration)) {
            self::$default_configuration = new APIRequestConfiguration();
        }
    ...
    public function __construct($search_params = array(), $configuration = NULL)
    {
        ...
        $person = !empty($search_params['person']) ? $search_params['person'] : new Person();
        ...
        if (!empty($search_params['first_name']) || !empty($search_params['middle_name']) || !empty($search_params['last_name'])) {
            ...
            $name = new NameField(array('first' => $first, 'middle' => $middle, 'last' => $last));
            $person->add_fields(array($name));
        }

        if (!empty($search_params['raw_name'])) {
            $person->add_fields(array(new NameField(array('raw' => $search_params['raw_name']))));
        }

        if (!empty($search_params['email'])) {
            $person->add_fields(array(new EmailField(array('address' => $search_params['email']))));
        }

        if (!empty($search_params['phone'])) {
            $person->add_fields(array(PhoneField::from_text($search_params['phone'])));
        }

        if (!empty($search_params['username'])) {
            $person->add_fields(array(new UsernameField(array('content' => $search_params['username']))));
        }

        if (!empty($search_params['user_id'])) {
            $person->add_fields(array(new UserIdField(array('content' => $search_params['user_id']))));
        }

        if (!empty($search_params['url'])) {
            $person->add_fields(array(new URLField(array('url' => $search_params['url']))));
        }
        ...
        if (!empty($search_params['country']) || !empty($search_params['state']) || !empty($search_params['city'])) {
            ...
            $address = new AddressField(array('country' => $country, 'state' => $state, 'city' => $city));
            $person->add_fields(array($address));
        }

        if (!empty($search_params['raw_address'])) {
            $person->add_fields(array(new AddressField(array('raw' => $search_params['raw_address']))));
        }

        if (!empty($search_params['from_age']) || !empty($search_params['to_age'])) {
            $dob = DOBField::from_age_range(!empty($search_params['from_age']) ? $search_params['from_age'] : 0,
    ...
    {
        // Send the request and return the response or raise SearchAPIException.
        //
        ...
        // Also raises an SearchAPIException object in case of an error
        //
    ...
    protected function executeRequest($url, $params)
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_VERBOSE => 0,
            CURLOPT_URL => $url,
            CURLOPT_USERAGENT => APIUtil::PIPLAPI_USERAGENT,
        ...
        if (in_array($http_status, range(200, 299))) {
            ...
            $err = SearchAPIException::from_array(
