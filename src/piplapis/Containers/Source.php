...
class Source extends Container
{
    ...
    private $extended_containers = array(
        'Pipl\Containers\Container' => 'relationships',
        'TagField' => 'tags'
    );
    public $name;
    public $category;
    ...
    public static function from_array($params)
    {
        ...
        if (!empty($valid_since)){ $valid_since = APIUtil::piplapi_str_to_datetime($valid_since); }
    ...
    public function to_array()
    {
        ...
        $d = array();
        if (!empty($this->valid_since)){ $d['@valid_since'] = APIUtil::piplapi_datetime_to_str($this->valid_since); }
